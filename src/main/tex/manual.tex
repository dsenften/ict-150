...
und muss entsprechend mit Abmahnungen rechnen \totalref{subsec:juristische-grundlagen}.
...
\index{Wertschöpfungskette}

\image{Wertschoepfungskette}{Wertschöpfungskette}
...
\image{eProcurement}{Marktmodelle eProcurement}
\label{MMeP}
...
Im Marketing gibt es die bekannte AIDA-Formel\index{AIDA-Formel}.
...
\begin{itemize}
    ...
    Dies geschieht mittels digitaler Signaturen \totalref{subsec:digitale-signatur}.
...
Rechnern, um damit wiederum Angriffe wie DoS-Attacken \totalref{subsub:dos}
zu starten. Ebenso
...
\totalref{subsec:xor} und einige
mathematische Grundlagen. Diese werden im Verfahren von Diffie/Hellmann und
RSA \totalref{subsec:rsa} angewendet.
Im letzten Abschnitt \totalref{subsec:java-programme} wird noch erklärt, wie die mathematischen
...
\totalref{subsubsec:monoalphabetische-substitution}, bei der die
Position bestehen bleibt und die Zeichen ersetzt werden.

\subsubsection*{Cäsar-Verfahren}
\index{Cäsar ! Verfahren von}
...
Hash-Funktionen \totalref{subsubsec:einfach} sind diese Funktionen umkehrbar.
...
\begin{bemerkung}[RSA]
    Für das RSA-Verfahren \totalref{subsec:rsa}
    und das Verfahren von Diffie/Hellmann \totalref{subsec:diffie-hellmann}
...
Einsatzgebiete:
\begin{itemize}
    \item Digitale Signatur \totalref{subsec:digitale-signatur}
    \item Digitaler Fingerabdruck
    \item Prüfsummenbildung
    \item Passwortlisten \totalref{subsubsec:password-cracker}
...
\begin{enumerate}
    ...
    (z.B.~mit dem XOR-Verfahren \totalref{subsec:xor}).
\end{enumerate}
...
%\index{Ubung@Übung!Warenkorb}
%
%Installieren Sie den Demo-Warenkorb. Gehen Sie wie folgt vor:
%\begin{enumerate}
% \item Download: Je nach Infrastruktur stellt Ihnen die Lehrperson ein ZIP-File mit PHP-Quellcode zur Verfügung. Extrahieren 		       Sie die Dateien auf Ihrem PC und installieren Sie die Applikation entsprechend.
% \item Kaufen Sie eine Ware (\texttt{http://localhost/wako/produkte.php})
%       und vergleichen Sie danach die Bestellungen\\
%       (\texttt{http://localhost/wako/admin.php?passwort=123456}).
% \item Versuchen Sie eine Session Ihres Nachbarn zu
%       \textit{hijacken}. Das geht hier ganz ohne Kenntnisse von TCP.
% \item Kaufen Sie einen Artikel, der nicht auf der Artikelliste zu
%       finden ist. Spionieren Sie als Kunde in der Datenbank: GET-Parameter
%       ausprobieren oder systematisch abholen.
% \item Betreiben Sie ``E-Shop-Lifting'': Kaufen Sie einen Artikel zu
%       verfälschtem Preis.
% \item Fügen Sie neue Produkte in die Produkteliste ein, und testen Sie
%       eine Bestellung.
%\end{enumerate}
...
%
%Geben Sie zu vier der folgenden möglichen Angriffe aus dem Kapitel
%Sicherheit \totalref{subsec:angriffe} eine mögliche Abhilfe.
%
%
